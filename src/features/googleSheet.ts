...
const clearSheetData = async (sheetName: string, sheetId: string) => {
  try {
    ...
    await sheets.spreadsheets.values.clear({ spreadsheetId: sheetId, range: `${sheetName}!A:F` });
...
export const getExistingDataFromSheet = async (
  ...
  try {
    ...
    console.log(
      `🖨️   ${filteredValues?.length} ransactions for ${sheetName} from Google Spreadsheet fetched`,
    );
...
export const cleanupGoogleSheets = async (sheetId: string): Promise<void> => {
  try {
    ...
    throw new Error(`❌  Error during cleanup (sheets): ${error.message}`, { cause: error });
  }
};
