...
import type { Transaction, TransactionObject, TransactionObjOptStr } from "../types";
...
const normalizeDates = (data: TransactionObjOptStr[]) =>
  data.map((item) => {
    ...
    if (regex.test(item.date as string)) {
...
): { expenses: Transaction[]; incomes: Transaction[]; investments: Transaction[] } => {
